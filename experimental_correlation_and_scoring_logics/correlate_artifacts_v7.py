#!/usr/bin/env python3
"""
correlate_artifacts_v6.py  -  Review & Hardened Version
Forensic artifact correlation -> per-user suspicion scoring

Changes from v5 (Review fixes):
  [FIX-A] norm_scaled: correct divisor (/ MAX_NORM_SCORE) — was always clipped to 100,
           destroying discrimination power of the normalization component.
  [FIX-B] evt_wc / 4 removed: event anomaly scoring now uses evt_wc directly
           scaled by WEIGHTS["event_anomalies"], consistent with other categories.
  [FIX-C] _assign_risk_labels: percentile index uses floor((n-1)*p) for correct
           inclusive percentile — prevents HIGH label from being unreachable.

Inherited from v5:
  [FIX-1] App scoring: per-user resolution via path + fallback to shared pool
  [FIX-2] Timeline bonus: capped multi-match (no early break)
  [FIX-3] Log gap: requires prior activity burst (>=3 events before gap)
  [FIX-4] Hybrid scoring: log1p scaling to dampen raw-score outliers
  [FIX-5] Evaluation: dynamic threshold (mean + 1σ, or top-N percentile)
  [NEW-6] Evidence diversity score (number of distinct artifact categories)
  [NEW-7] Risk classification: HIGH / MEDIUM / LOW labels

Usage:
    python3 correlate_artifacts_v6.py --json-dir output/json --output output/scores.json
    python3 correlate_artifacts_v6.py --json-dir output/json --output output/scores.json \
        --ground-truth ground_truth.json --threshold-method sigma
"""
...
# [FIX-A] Maximum theoretical norm_score = sum of all weights (when one category
# has all events). Used to correctly scale norm_score to [0, 100].
# Sum = 4+4+3+3+2+1+1 = 18
MAX_NORM_SCORE: float = float(sum(WEIGHTS.values()))  # 18.0
...
# [FIX-2] Maximum bonus per pattern type to prevent inflation
TIMELINE_BONUS_CAP = {
    "file_access_then_deletion":  15,   # up to 3 occurrences
    "app_exec_then_network":      12,   # up to 3 occurrences
    "activity_then_log_gap":       6,   # once is enough (strong signal)
    "rapid_actions":               9,   # up to 3 bursts
    "multi_source_consistency":   10,   # up to 2 occurrences
}

# Hybrid ratio (now applied to log-scaled values - see [FIX-4])
RAW_WEIGHT  = 0.70
NORM_WEIGHT = 0.30
...
def _username_from_path(path: str) -> str | None:
    r"""Extract username from Windows path (Users\username or Documents and Settings\username)."""
    if not path:
        return None
    m = re.search(
...
# [FIX-4] log1p scaling to dampen outlier raw scores
def log_scale(value: float, max_val: float) -> float:
    """Scale value to [0, 100] using log1p to reduce outlier dominance."""
    if max_val <= 0:
        return 0.0
...
def score_deleted_files(data: dict | None, sid_map: dict | None = None) -> dict:
    ...
    for rec in data.get("records", []):
        user = resolve_username(rec.get("original_path", ""),
                                rec.get("sid"), sid_map)
        if not user:
            # [FIX-6] Track unresolved records instead of silently dropping
            user = "__unknown__"
            unresolved += 1
        scores[user]["count"] += 1
...
# [FIX-1] Per-user app scoring via path resolution
def score_application_activity(data: dict | None, sid_map: dict | None = None) -> dict:
    """
    Attempt per-user attribution via exe path.
    Falls back to __apps__ pool for system-wide prefetch entries.
    __apps__ is distributed proportionally in aggregate_scores.
    """
    ...
    for rec in data.get("records", []):
        ...
        # Check if this exe is suspicious
        for kw, (cat, mult) in SUSPICIOUS_EXES.items():
            if kw in exe:
                # Try user attribution from path
                user = resolve_username(exe_path, rec.get("sid"), sid_map)
                if not user:
                    user = "__apps__"   # system-wide pool
                    shared_count += 1
                else:
                    attributed_count += 1
...
def _build_timeline(user: str, doc_s: dict, del_s: dict,
    ...
    # [FIX-D] Only user-attributed apps enter the timeline.
    # Shared __apps__ pool has no user attribution — injecting it into every
    # user's timeline caused Pattern 4 (rapid_actions) to fire for ALL users,
    # giving every account +15 timeline bonus regardless of actual behaviour.
...
# =============================================================================
# TIMELINE CORRELATION - [FIX-2] Capped multi-match, [FIX-3] Gap heuristic
# =============================================================================
...
def calculate_timeline_bonuses(
    ...
    # [FIX-E] Require actual artifact evidence before awarding timeline bonus.
    # A user with artifact_score == 0 has no per-user evidence at all.
    # Awarding timeline bonus to them would be forensically meaningless.
    ...
    # [FIX-3] Require at least 3 events before the gap to filter idle periods
    BURST_WINDOW_SEC = 3600     # 1 hour window to count "activity before gap"
    MIN_PRE_GAP_EVENTS = 3      # minimum events required before the gap

    for i in range(len(timeline) - 1):
        gap = (timeline[i+1]["ts"] - timeline[i]["ts"]).total_seconds()
        if gap >= 3600:
            # Count events in the hour before this gap
            ...
            if len(pre_gap_events) >= MIN_PRE_GAP_EVENTS:
                ...
                if key not in used:
                    ...
                    break   # one gap bonus is sufficient forensically

    # ── Pattern 4: Rapid actions - >= 5 events within 60 s ───────────────────
    # [FIX-2] Multi-match allowed up to cap
...
# =============================================================================
# [NEW-6] EVIDENCE DIVERSITY SCORE
# Measures how many different artifact categories contributed evidence.
# A high-scoring user who only has deleted files is less convincing than one
# with evidence across 5 categories.
# =============================================================================
...
def compute_diversity_score(
    ...
    """
    Returns:
      category_count: number of categories with any evidence (0-7)
      diversity_score: normalized [0, 1.0]
      categories_hit: list of category names that contributed
    """
    categories = {
...
# =============================================================================
# AGGREGATE  -  two-pass hybrid scoring with log1p [FIX-4]
# =============================================================================
...
def aggregate_scores(
    ...
    # [FIX-1] Also collect per-user app keys
    ...
    for user in all_keys:
        ...
        # [FIX-1] Per-user attributed apps + shared pool portion
        ...
        app_share   = app_user_wc + ((app_shared / num_users) if has_act else 0)

        raw[user] = (
            del_c   * WEIGHTS["deleted_files"]
            + evt_wc * WEIGHTS["event_anomalies"]   # [FIX-B] was (evt_wc/4)*weight
    ...
    for user in sorted(all_keys):
        ...
        # [FIX-A+] Include app_share in total_ev so norm(app_share) stays in [0,1]
        # Without this, large app_share / small total_ev -> norm >> 1 -> norm_scaled >> 100
        total_ev = max(del_c + evt_c + net_c + doc_sc + brw_fc + usr_c + app_share, 1)
        def norm(v: float) -> float: return v / total_ev

        norm_score = (
            WEIGHTS["deleted_files"]    * norm(del_c)
            + WEIGHTS["event_anomalies"]  * norm(evt_wc)   # [FIX-B] was norm(evt_wc) -- OK here, /4 was only in Pass1
        ...
        # [FIX-4] Log-scaled hybrid: log1p dampens outlier dominance
        raw_scaled  = log_scale(raw_score, max_raw)
        # [FIX-A] Correct normalization: divide by theoretical maximum (18.0)
        # Previous: norm_score * 100 could reach 1800, clipped to 100 for everyone
        # Fixed:    norm_score / MAX_NORM_SCORE * 100 stays in [0, 100] naturally
        norm_scaled = (norm_score / MAX_NORM_SCORE) * 100.0
        artifact_score = RAW_WEIGHT * raw_scaled + NORM_WEIGHT * norm_scaled

        # Timeline bonus — [FIX-E] pass artifact_score to gate bonus on real evidence
        ...
        # [NEW-6] Evidence diversity
        diversity = compute_diversity_score(
            del_c, evt_c, app_share, net_c, doc_sc, brw_fc, usr_c
        )
        ...
        results.append({
            ...
            "artifact_scores": {
                ...
                "event_anomalies":  {"raw_count": evt_c,
                                     ...
                                     "score": round(evt_wc * WEIGHTS["event_anomalies"], 2)},  # [FIX-B]
                "app_activity":     {
            ...
            "diversity":         diversity,       # [NEW-6]
            "risk_label":        None,            # [NEW-7] set in post-processing
            "evidence": {
    ...
    # [NEW-7] Assign risk labels using percentile thresholds
...
# =============================================================================
# [NEW-7] RISK CLASSIFICATION
# =============================================================================

def _assign_risk_labels(ranked_users: list[dict]) -> None:
    """
    Assigns HIGH / MEDIUM / LOW labels based on score distribution.

    Uses inclusive percentile index: floor((n-1) * p)
    This guarantees the top user always reaches HIGH (p=0.80 -> index n-1 when n=1).
      - TOP 20%  -> HIGH
      - 20-50%   -> MEDIUM
      - BOTTOM 50% -> LOW

    Falls back to absolute thresholds if fewer than 3 users.
    """
    if not ranked_users:
        return

    scores = [r["final_score"] for r in ranked_users]
    n = len(scores)

    if n < 3:
        for r in ranked_users:
            s = r["final_score"]
            r["risk_label"] = "HIGH" if s > 50 else ("MEDIUM" if s > 20 else "LOW")
        return

    scores_sorted = sorted(scores)
    # [FIX-C] Inclusive percentile: floor((n-1) * p)
    # [FIX-G] Use STRICT inequality for p80 boundary to prevent tied-score inflation.
    # When many users share the same score (e.g. all 0 except rank-1),
    # p80 equals that shared score, so >= would make everyone HIGH.
    # Rule: score > p80 threshold -> HIGH (strictly above)
    #       score == p80 only qualifies if the user is genuinely in top 20% by rank.
    p80 = scores_sorted[math.floor((n - 1) * 0.80)]
    p50 = scores_sorted[math.floor((n - 1) * 0.50)]

    # [FIX-G] Rank-gated label assignment.
    # At a tie boundary, only the top-ranked users within the quota get the
    # higher label. This prevents "all share the same score -> all get HIGH".
    # Quota: at least 1 user in HIGH; at least 1 in MEDIUM (when n >= 2).
    top20_count = max(1, round(n * 0.20))   # HIGH quota
    top50_count = max(2, round(n * 0.50))   # HIGH+MEDIUM combined quota

    # ranked_users is already sorted highest score first (rank 1 = highest)
    for r in ranked_users:
        rnk = r.get("rank", n)   # rank is 1-based
        if rnk <= top20_count:
            r["risk_label"] = "HIGH"
        elif rnk <= top50_count:
            r["risk_label"] = "MEDIUM"
        else:
            r["risk_label"] = "LOW"

# =============================================================================
# EVALUATION  -  [FIX-5] Dynamic threshold
# =============================================================================

def evaluate_results(results: list[dict], ground_truth: dict,
                     threshold_method: str = "sigma") -> dict:
    """
    ground_truth: {"username_key": true/false, ...}

    threshold_method:
      "positive"   - original: score > 0 (too lenient)
      "sigma"      - mean + 1 standard deviation (default)
      "percentile" - top 20% of scores
      "manual"     - user-supplied; pass threshold kwarg (not implemented here)
    """
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Correlate forensic artifacts -> per-user suspicion scores (v5)"
    )
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print("  ║       Forensic Artifact Correlator - v7             ║")
    print("  ║  shared-pool-fix · gate-fix · tie-fix · label-fix  ║")
    print("  ╚══════════════════════════════════════════════════════╝")
    print(f"\n  JSON dir : {json_dir}\n  Output   : {out_path}\n")
    ...
    app_s = score_application_activity(app_data, sid_map)   # [FIX-1] now takes sid_map
    ...
    output = {
        "metadata": {
            ...
            "scoring_method":         f"log1p_hybrid_{int(RAW_WEIGHT*100)}_{int(NORM_WEIGHT*100)}_normfix",
            ...
            "app_attribution":        "per-user path-based + shared pool fallback",
            ...
            "changes_from_v6": [
                "FIX-D: __apps__ shared pool REMOVED from per-user timelines (caused all-HIGH)",
                "FIX-E: Timeline bonus gated on artifact_score > 0 (no evidence = no bonus)",
                "FIX-G: Risk label tie-breaking uses rank to prevent boundary inflation",
            ],
            "changes_from_v5": [
                "FIX-A: norm_scaled now uses /MAX_NORM_SCORE (was *100 then clipped — always 100)",
                "FIX-B: evt_wc/4 divisor removed; event anomaly uses full weighted count",
                "FIX-C: Risk label percentile uses floor((n-1)*p) for correct inclusive index",
            ],
            "changes_from_v4": [
                "FIX-1: App scoring per-user via path resolution",
                "FIX-2: Timeline bonus capped multi-match (no early break)",
                "FIX-3: Log gap requires >=3 prior events to avoid idle FP",
                "FIX-4: log1p scaling for raw score (outlier dampening)",
                "FIX-5: Dynamic evaluation threshold (sigma/percentile)",
                "NEW-6: Evidence diversity score (category spread)",
                "NEW-7: HIGH/MEDIUM/LOW risk classification",
            ],
        },
        "users":      results,
