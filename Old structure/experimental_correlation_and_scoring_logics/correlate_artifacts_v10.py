#!/usr/bin/env python3
"""
correlate_artifacts_v10.py  -  Review & Hardened Version
Forensic artifact correlation -> per-user suspicion scoring

Changes from v9:
  [FIX-10A] Risk label assignment: added _has_real_evidence() to distinguish
            users with REAL evidence (deleted_files, event_anomalies, etc.)
            from users who only have shared pool app_activity.
  [FIX-10B] Users with only shared pool app_activity are now classified as LOW,
            regardless of final_score. Only users with real evidence can be
            elevated to HIGH/MEDIUM.
  [FIX-10C] _assign_risk_labels now uses real_evidence users only for
...
Usage:
    python3 correlate_artifacts_v10.py --json-dir output/json --output output/scores.json
    python3 correlate_artifacts_v10.py --json-dir output/json --output output/scores.json \
...
# =============================================================================
# [FIX-10A] REAL EVIDENCE DETECTION
# =============================================================================
...
def _has_real_evidence(user_data: dict) -> bool:
    ...
    if scores.get("document_access", {}).get("raw_count", 0) > 0:
        return True
    if scores.get("browser_history", {}).get("raw_count", 0) > 0:
        return True
...
def aggregate_scores(
    ...
    # [FIX-10B/C] Assign risk labels using REAL EVIDENCE gate
...
# =============================================================================
# [FIX-10B/C] RISK CLASSIFICATION - REAL EVIDENCE GATE
# =============================================================================
...
def _assign_risk_labels(ranked_users: list[dict]) -> None:
    ...
    [FIX-10B] Users with ONLY shared pool app_activity are classified as LOW.
              Only users with REAL evidence (deleted_files, event_anomalies,
              document_access, browser_history, user_accounts, network_activity,
              or attributed app_activity) can be elevated to HIGH/MEDIUM.

    [FIX-10C] Percentile threshold calculation uses ONLY users with real evidence.

    Rule 1 (Zero guard):  artifact_score == 0 → LOW unconditionally.

    Rule 2 (Real evidence gate): Only users with _has_real_evidence() == True
                                 are considered for HIGH/MEDIUM classification.

    Rule 3 (Distribution): among real evidence users, apply percentile thresholds:
      - TOP 20%    → HIGH
      - 20 – 50%   → MEDIUM
      - BOTTOM 50% → LOW
    """
    ...
    if n == 2:
        # Higher score → HIGH, lower → MEDIUM
        real_evidence_users[0]["risk_label"] = "HIGH"
        real_evidence_users[1]["risk_label"] = "MEDIUM"
        return
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Correlate forensic artifacts -> per-user suspicion scores (v10)"
    )
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print("  ║       Forensic Artifact Correlator - v10            ║")
    print("  ║    zero-guard · evidence-gated risk classification  ║")
    print("  ║    + real-evidence gate · shared-pool-all-users     ║")
    print("  ╚══════════════════════════════════════════════════════╝")
    print(f"\n  JSON dir : {json_dir}\n  Output   : {out_path}\n")
    ...
    output = {
        "metadata": {
            ...
            "real_evidence_gate":     "users with ONLY shared app pool → LOW",
            ...
            "changes_from_v9": [
                "FIX-10A: Risk label assignment: added _has_real_evidence() to distinguish",
                "         users with REAL evidence from users who only have shared pool",
                "FIX-10B: Users with only shared pool app_activity are now classified as LOW,",
                "         regardless of final_score. Only users with real evidence can be",
                "         elevated to HIGH/MEDIUM.",
                "FIX-10C: _assign_risk_labels now uses real_evidence users only for",
            ...
            "changes_from_v6": [
                "FIX-D: __apps__ shared pool REMOVED from per-user timelines",
                "FIX-E: Timeline bonus gated on artifact_score > 0",
                "FIX-G: Risk label tie-breaking uses rank to prevent boundary inflation",
            ],
            "changes_from_v5": [
                "FIX-A: norm_scaled now uses /MAX_NORM_SCORE",
                "FIX-B: evt_wc/4 divisor removed",
                "FIX-C: Risk label percentile uses floor((n-1)*p)",
            ],
            "changes_from_v4": [
                "FIX-1: App scoring per-user via path resolution",
                "FIX-2: Timeline bonus capped multi-match",
                "FIX-3: Log gap requires >=3 prior events",
                "FIX-4: log1p scaling for raw score",
                "FIX-5: Dynamic evaluation threshold",
