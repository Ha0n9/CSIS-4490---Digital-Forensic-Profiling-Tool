#!/usr/bin/env python3
"""
correlate_artifacts_v9.py  -  Review & Hardened Version
Forensic artifact correlation -> per-user suspicion scoring

Changes from v8:
  [FIX-8A] Network activity: added score_network_from_browser() to extract
           network indicators from flagged browser URLs (gunbroker, etc.)
  [FIX-8B] App activity shared pool: now distributed to ALL users, not only
           those with has_act (fixes Administrator/Guest having 0 app score)
  [FIX-8C] Timeline bonus gating: now uses _has_any_evidence() instead of
           artifact_score > 0 (prevents false negatives from log1p rounding)
  [FIX-8D] Browser history weight: reduced from 1.0 to 0.5 to prevent
           browser history from dominating scores
  [FIX-8E] Network classification: added cloud storage detection for
           OneDrive, Google Drive, Dropbox, Box, AWS S3

Inherited from v8:
  [FIX-H] Risk labels score-gated: score==0 → LOW unconditionally
  [FIX-H] Percentile applied only to users with score > 0 (real evidence)

Usage:
    python3 correlate_artifacts_v9.py --json-dir output/json --output output/scores.json
    python3 correlate_artifacts_v9.py --json-dir output/json --output output/scores.json \
...
# [FIX-8D] Reduced browser_history weight from 1.0 to 0.5
WEIGHTS = {
    "deleted_files":    4,
    "event_anomalies":  4,
    "app_activity":     3,
...
# [FIX-8E] Cloud storage domains for network classification
...
def classify_network(dest_ip: str, dest_port: str | int, process: str, url: str = "") -> str:
    ...
    # [FIX-8E] Check for cloud storage in URL
    if url:
        url_lower = url.lower()
...
SUSPICIOUS_DOMAINS: dict[str, tuple[str, int]] = {
    ...
    "shodan":       ("hacking", 2),
    # [FIX-8A] Additional domains that should trigger network flags
...
def score_network_activity(data: dict | None) -> dict:
    ...
    for evt in data.get("network_events", []):
        ...
        # [FIX-8E] Enhanced classification with URL
...
# [FIX-8A] Network indicators from browser history
def score_network_from_browser(data: dict | None) -> dict:
    """
    Extract network indicators from browser history flagged URLs.
    Counts visits to suspicious domains as network activity.
    """
    scores: dict = defaultdict(lambda: {
        "count": 0.0, "raw_count": 0, "external": 0, "internal_suspicious": 0,
        "account_type": "user", "evidence": [],
    ...
    for rec in data.get("records", []):
        ...
        for kw, (cat, weight) in SUSPICIOUS_DOMAINS.items():
            if kw in url or kw in title:
                # Weighted network activity - scale factor 0.3 to avoid double-counting
                # since browser_history already counts this
...
# [FIX-8C] Check if user has any evidence at all (for timeline gating)
def _has_any_evidence(user: str, doc_s: dict, del_s: dict,
                      app_s: dict, net_s: dict, evt_s: dict) -> bool:
    """Check if user has any evidence from any artifact."""
    return (
...
def calculate_timeline_bonuses(
    ...
    # [FIX-8C] Use _has_any_evidence() instead of artifact_score > 0
    # This prevents false negatives from log1p rounding artifacts_score to 0
...
def aggregate_scores(
    ...
    net_brw_s: dict,  # [FIX-8A] network from browser
) -> list[dict]:
    ...
    for user in all_keys:
        ...
        raw[user] = (
            ...
            + (net_c + net_brw) * WEIGHTS["network_activity"]  # [FIX-8A] include browser network
            + doc_sc       * WEIGHTS["document_access"]
            + brw_fw       * WEIGHTS["browser_history"]  # [FIX-8D] weight reduced to 0.5
    ...
    for user in sorted(all_keys):
        ...
        app_user_wc = app_s.get(user, {}).get("weighted_count", 0.0)
        app_shared  = app_s.get("__apps__", {}).get("weighted_count", 0.0)
        # [FIX-8B] Remove has_act condition
        app_share   = app_user_wc + (app_shared / num_users)

        net_total = net_c + net_brw  # [FIX-8A]
        ...
        norm_score = (
            ...
            + WEIGHTS["network_activity"] * norm(net_total)  # [FIX-8A]
            + WEIGHTS["document_access"]  * norm(doc_sc)
            + WEIGHTS["browser_history"]  * norm(brw_fw)  # [FIX-8D] weight reduced
            + WEIGHTS["user_accounts"]    * norm(usr_c)
        )

        # [FIX-4] Log-scaled hybrid
        raw_scaled  = log_scale(raw_score, max_raw)
        # [FIX-A] Correct normalization: divide by theoretical maximum
        norm_scaled = (norm_score / MAX_NORM_SCORE) * 100.0
        artifact_score = RAW_WEIGHT * raw_scaled + NORM_WEIGHT * norm_scaled

        # Timeline bonus — [FIX-8C] uses _has_any_evidence() internally
        ...
        results.append({
            ...
            "artifact_scores": {
                ...
                "browser_history":  {"raw_count": brw_t, "flagged_count": brw_fc,
                                     ...
                                     "score": round(brw_fw * WEIGHTS["browser_history"], 2)},  # [FIX-8D]
...
def _assign_risk_labels(ranked_users: list[dict]) -> None:
    """
    Assigns HIGH / MEDIUM / LOW labels based on score AND score distribution.

    Rule 1 (Zero guard):  score == 0  →  LOW unconditionally.
                          No evidence means no suspicion, regardless of rank.

    Rule 2 (Distribution): among users with score > 0, apply percentile thresholds:
      - TOP 20%    → HIGH
      - 20 – 50%   → MEDIUM
      - BOTTOM 50% → LOW

    Rationale: ranking users with identical zero scores into HIGH/MEDIUM is
    forensically meaningless. Only users with actual evidence can be elevated.
    """
    if not ranked_users:
        return

    # [FIX-H] Separate users who have real evidence from those who do not
    with_evidence    = [r for r in ranked_users if r["final_score"] > 0]
    without_evidence = [r for r in ranked_users if r["final_score"] <= 0]

    # Users with no evidence are always LOW
    for r in without_evidence:
        r["risk_label"] = "LOW"

    # If nobody has any evidence, we are done
    ...
    if n == 1:
        # Only one user with evidence — they are HIGH by definition
        with_evidence[0]["risk_label"] = "HIGH"
        return

    if n == 2:
        # Two users: higher score → HIGH, lower → MEDIUM
        with_evidence[0]["risk_label"] = "HIGH"
        with_evidence[1]["risk_label"] = "MEDIUM"
        return

    # n >= 3: use score-value percentile thresholds (standard inclusive index)
    scores_sorted = sorted(r["final_score"] for r in with_evidence)
    p80 = scores_sorted[math.floor((n - 1) * 0.80)]   # 80th percentile value
    p50 = scores_sorted[math.floor((n - 1) * 0.50)]   # 50th percentile value
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Correlate forensic artifacts -> per-user suspicion scores (v9)"
    )
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print("  ║       Forensic Artifact Correlator - v9             ║")
    print("  ║    zero-guard · evidence-gated risk classification  ║")
    print("  ║    + network-from-browser · shared-pool-all-users   ║")
    print("  ╚══════════════════════════════════════════════════════╝")
    print(f"\n  JSON dir : {json_dir}\n  Output   : {out_path}\n")
