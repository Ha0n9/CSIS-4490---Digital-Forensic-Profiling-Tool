#!/usr/bin/env python3
"""
correlate_artifacts_v3.3.py
Production-ready version with balanced scoring and realistic rankings.
"""
...
# =============================================================================
# WEIGHTS - Realistic for production
# =============================================================================

WEIGHTS = {
    "deleted_files":    5,    # High - evidence of cover-up
    "event_anomalies":  3,    # Medium - could be normal
    "app_activity":     2,    # Medium - suspicious but could be legitimate
    "network_activity": 1,    # Low - too many false positives
    "document_access":  4,    # High - sensitive data access
    "browser_history":  2,    # Medium - context only
    "user_accounts":    1,    # Low - account flags
}

TIMELINE_BONUS = {
    "file_access_then_deletion":  5,
    "app_exec_then_network":      3,
    "activity_then_log_gap":      4,
    "rapid_actions":              2,
...
SYSTEM_ACCOUNTS: Set[str] = {
    ...
    "administrator", "guest", "defaultaccount",  # Built-in accounts with low risk
    "wdagutilityaccount",
}
...
def score_event_logs(data: Optional[Dict]) -> Dict:
    """Weight 3 - Count anomalous events per user."""
    scores = defaultdict(lambda: {"count": 0, "weighted_count": 0.0, "evidence": []})
    if not data:
        return dict(scores)
    ...
    for evt in data.get("all_events", []):
        ...
        # Extract username from multiple fields
        uname = (
        ...
        # Skip system accounts only
        if uname and is_system_account(uname):
            continue
        
        # Only include if we have a valid username
        if not uname:
            continue
...
def score_network_activity(data: Optional[Dict]) -> Dict:
    """
    Weight 1 - Count all network events, mark internal vs external.
    """
    scores = defaultdict(lambda: {"count": 0, "external_count": 0, "evidence": []})
    if not data:
        return dict(scores)
    ...
    for evt in data.get("network_events", []):
        ...
        is_external = not is_internal_ip(dest_ip) if dest_ip else False
        ...
        is_suspicious = bool(process and any(exe in process for exe in SUSPICIOUS_EXES.keys()))
        
        # Count all traffic, but mark external separately
        scores[key]["count"] += 1
        if is_external:
            scores[key]["external_count"] += 1
...
def aggregate_scores(users: Dict, del_s: Dict, app_s: Dict, evt_s: Dict,
    ...
    """
    V3.3: Realistic scoring with proper scaling.
    """
    ...
    for user in sorted(all_keys):
        ...
        net_count = net_s.get(user, {}).get("external_count", 0)  # Only external
        ...
        # App activity (distributed)
        app_wcount = app_s.get("__system_apps__", {}).get("weighted_count", 0.0)
        has_activity = (del_count > 0 or evt_wcount > 0 or net_count > 0 or 
    ...
    for user in sorted(all_keys):
        ...
        # Skip built-in accounts with zero activity
        if display_name.lower() in ("administrator", "guest", "defaultaccount", 
                                     "wdagutilityaccount"):
            # Still include them in output but with zero score
            pass
        
        del_count = del_s.get(user, {}).get("count", 0)
        evt_count = evt_s.get(user, {}).get("count", 0)
        evt_wcount = evt_s.get(user, {}).get("weighted_count", 0.0)
        net_count = net_s.get(user, {}).get("external_count", 0)
        net_total = net_s.get(user, {}).get("count", 0)
        ...
        # HYBRID: 80% raw (normalized to 0-100) + 20% normalized
        if max_raw > 0:
            raw_normalized = (raw_score / max_raw) * 100
        else:
            raw_normalized = 0
            
        artifact_score = (0.8 * raw_normalized) + (0.2 * norm_score)
        ...
        if raw_score == 0 and timeline_bonus == 0:
            continue
