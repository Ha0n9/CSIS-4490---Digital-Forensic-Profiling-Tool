...
def score_event_logs(data: Optional[Dict]) -> Dict:
    """Weight 4 - Count anomalous events per user with balanced filtering."""
    scores = defaultdict(lambda: {"count": 0, "weighted_count": 0.0, "evidence": []})
    if not data:
        return dict(scores)
    ...
    for evt in data.get("all_events", []):
        ...
        # Only filter known system accounts
        if uname and is_system_account(uname):
            continue
        
        key = normalize_username(uname) if uname else "unresolved"
...
def score_network_activity(data: Optional[Dict]) -> Dict:
    """Weight 2 - Count network events with balanced filtering."""
    scores = defaultdict(lambda: {"count": 0, "evidence": []})
    if not data:
        return dict(scores)
...
def aggregate_scores(users: Dict, del_s: Dict, app_s: Dict, evt_s: Dict,
    ...
    """
    V3.1: Simplified aggregation - use raw scoring with system filtering.
    """
    # Collect all valid user keys (exclude system and unresolved)
    all_keys: Set[str] = set(users.keys())
    for d in (del_s, evt_s, net_s, doc_s, brw_s, usr_s):
        all_keys.update(d.keys())
    
    # Filter out system accounts and unresolved
    all_keys = {k for k in all_keys 
                if not is_system_account(k) 
                and k not in ("unresolved", "unknown")}
    ...
    for user in sorted(all_keys):
        ...
        # NORMALIZED SCORING (V2 style) - for reference only
