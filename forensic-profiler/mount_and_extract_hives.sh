...
# and 0 for logical/single-volume acquisitions with no partition table at all)
if [[ "$MOUNT_SUCCESS" == false ]]; then
    log_warn "Auto-detect failed — trying common offsets..."
    for FALLBACK_OFFSET in 1259520 2048 206848 1026048 63 0; do
        # Skip if already tried (only meaningful when auto-detect actually found an offset —
        # an empty WINDOWS_OFFSET must never be treated as "already tried offset 0")
        if [[ -n "$WINDOWS_OFFSET" ]]; then
            _fb_dec=$(echo "$FALLBACK_OFFSET" | sed 's/^0*//'); _fb_dec=${_fb_dec:-0}
            _wd_dec=$(echo "$WINDOWS_OFFSET" | sed 's/^0*//'); _wd_dec=${_wd_dec:-0}
...

{
    ...
    echo "  Windows Ver    : $WIN_VER"   # ← FIX: Explicitly write Windows version to report
