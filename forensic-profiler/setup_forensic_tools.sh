...
# =============================================================================
# STEP 2: Python packages (critical for parsers)
# =============================================================================
...
for pkg in "${PYTHON_PACKAGES[@]}"; do
    ...
    if pip3 install "$pkg" --break-system-packages 2>/dev/null; then
        ...
        if pip3 install "$pkg" 2>/dev/null; then
            log_success "  $pkg installed (fallback)"
        else
...
PKG_OK=true
for pkg in Registry evtx pylnk3; do
    if python3 -c "import $pkg" 2>/dev/null; then
        log_success "  $pkg OK"
    else
        log_warn "  $pkg NOT INSTALLED — parsers may fail"
        PKG_OK=false
    fi
done

if [ "$PKG_OK" = false ]; then
    log_warn "Some Python packages are missing. Try installing manually:"
    log_warn "  pip3 install python-registry evtx pylnk3 --break-system-packages"
fi
...
echo ""
log_info "Python packages (critical for parsers):"
check_python "python-registry (SAM/NTUSER parser)" "Registry"
check_python "evtx (event log parser)" "evtx"
check_python "pylnk3 (LNK parser)" "pylnk3"
...
echo ""
echo -e "  Quick test         :"
echo -e "  ${GREEN}python3 -c \"from Registry import Registry; print('Registry OK')\"${NC}"
echo -e "  ${GREEN}python3 -c \"import evtx; print('evtx OK')\"${NC}"
echo -e "  ${GREEN}python3 -c \"import pylnk3; print('pylnk3 OK')\"${NC}"
