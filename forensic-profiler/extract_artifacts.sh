...
#
# Usage:
#   bash extract_artifacts.sh -o <output_dir> [-m <mount_point>]
#
#   -o  Output directory (required) — must contain raw/ from mount_and_extract_hives.sh
#   -m  Mount point (optional) — only needed as fallback if extraction_report.txt is missing
# =============================================================================
...
usage() {
    echo -e "
${BOLD}Usage:${NC}
  bash $(basename "$0") -o <output_dir> [-m <mount_point>]
...
${BOLD}Examples:${NC}
  bash $(basename "$0") -o ./output
  bash $(basename "$0") -m /mnt/img_12345 -o ./output
"
    exit 0
}
...
if [[ -z "$OUTPUT" ]]; then
    log_error "Missing output directory!"; usage
fi

# Mount point is optional - warn if provided but doesn't exist
if [[ -n "$MOUNT" && ! -d "$MOUNT" ]]; then
    log_warn "Mount point not found: $MOUNT (ignoring, will use extraction report)"
    MOUNT=""
fi
...
echo -e "  Mount   : ${YELLOW}${MOUNT:-<not provided, using report>}${NC}"
echo -e "  EZ Tools: $([ "$HAS_EZ" = true ] && echo "${GREEN}available (will use as supplement)${NC}" || echo "${YELLOW}not found (Python-only mode)${NC}")"
echo ""

# ── Read Windows version from extraction_report.txt ───────────────────────────
# FIX: Prefer report over live detection to avoid re-scanning the mount point
WIN_VER="unknown"
USERS_DIR=""

REPORT_FILE="$OUTPUT/extraction_report.txt"
if [[ -f "$REPORT_FILE" ]]; then
    WIN_VER=$(grep -i "Windows Ver" "$REPORT_FILE" 2>/dev/null | awk -F':' '{print $2}' | tr -d ' ')
    if [[ -n "$WIN_VER" ]]; then
        log_info "Windows version from report: $WIN_VER"
    else
        WIN_VER="unknown"
    fi
fi

# Fallback: if not found in report AND mount point exists, detect from mount point
if [[ "$WIN_VER" == "unknown" && -n "$MOUNT" && -d "$MOUNT" ]]; then
    log_warn "Could not read Windows version from report, detecting from mount point..."
    ...
    # If read from report, set USERS_DIR accordingly (if mount exists)
    if [[ -n "$MOUNT" && -d "$MOUNT" ]]; then
        if [[ "$WIN_VER" == "xp" ]]; then
            USERS_DIR="$MOUNT/Documents and Settings"
        elif [[ "$WIN_VER" == "modern" ]]; then
            USERS_DIR="$MOUNT/Users"
        fi
    fi
fi
...
# Ensure WIN_VER is set
if [[ -z "$WIN_VER" || "$WIN_VER" == "unknown" ]]; then
    log_warn "Could not detect Windows version, defaulting to 'modern'"
    WIN_VER="modern"
fi

export WIN_VER MOUNT OUTPUT RAW JSON_DIR USERS_DIR

log_info "Using Windows version: $WIN_VER"
